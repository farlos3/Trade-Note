...
| `src/views/Mindset.vue` | **Mindset** page (Journal section). Principles the trader sets themselves, walked as a Duolingo-style path: oldest first, each a stage that is `todo` / `active` / `mastered` (only one `active` at a time). Node coordinates computed in JS; the trail is an SVG through the same points. Own `mindsets` class — not date-anchored like `notes`. CRUD in `src/utils/mindset.js`. |
